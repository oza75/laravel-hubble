<template>
    <div class="container dashboard--container dashboard--details">
        <header class="header">
            <h2 class="title">Détails - {{resource.title}}</h2>
            <div class="ctas">
                <a :href="item['@urls']['edit']['url']" class="btn btn-normal btn-primary btn-radius" title="Modifier">
        ...
        <v-modal label="Confirmation" @close="openDeleteModal = false" v-if="openDeleteModal">
            <template v-slot:body>
                <p>Voulez-vous vraiment supprimer cet élément.</p>
            </template>
            <template v-slot:footer>
                <button @click="openDeleteModal = false" class="btn btn-normal btn-text">Annuler</button>
                <form :action="item['@urls']['show']['url']" method="post">
                    <input type="hidden" name="_token" :value="resource.token">
                    <input type="hidden" name="_method" value="DELETE">
                    <button type="submit" class="btn btn-normal btn-primary">Supprimer</button>
                </form>
            </template>
        </v-modal>
